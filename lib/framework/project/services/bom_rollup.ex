defmodule Framework.Project.Services.BOMRollup do
  @moduledoc false
  ...
  alias Framework.Project.BOMRollup
  ...
  def refresh!(%BOM{} = bom, opts \\ []) do
    ...
    bom = Ash.load!(bom, [:item_id], actor: actor, authorize?: authorize?)
        ...
        case BOMRollup
          ...
          {:error, _e} ->
            case get_rollup(bom, actor, authorize?) do
              %BOMRollup{} = existing ->
                existing
                |> Ash.Changeset.for_update(:update, Map.drop(params, [:bom_id, :item_id]))
  ...
  defp flatten_components(%BOM{} = bom, quantity, opts) do
    ...
    bom =
      Ash.load!(bom, [components: [:component_type, :quantity, :item, :material]],
        actor: actor,
        authorize?: authorize?
      )
  ...
  @spec do_flatten(BOM.t(), D.t(), MapSet.t(Project.item_id()), keyword()) :: %{any() => D.t()}
            ...
            %{id: item_id} ->
              if MapSet.member?(path, item_id) do
                ...
                if nested_bom do
                  ...
                  nested_map =
                    do_flatten(nested_bom, comp_qty, MapSet.put(path, item_id), opts)
