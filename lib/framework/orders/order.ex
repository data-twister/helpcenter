defmodule Framework.Orders.Order do
  @moduledoc false
  use Ash.Resource,
    otp_app: :framework,
  ...
  alias Framework.Orders.Changes.CalculateTotals
  alias Framework.Orders.Changes.ValidateConstraints
  ...
  actions do
    ...
    read :list do
      prepare build(sort: [delivery_date: :asc], load: [:customer, items: [:item]])
      ...
      filter expr(is_nil(^arg(:item_id)) or exists(items, item_id == ^arg(:item_id)))
  ...
  attributes do
    ...
    attribute :currency, Framework.Types.Currency do
      allow_nil? false
      default :USD
    end
  ...
  relationships do
    ...
    belongs_to :customer, Framework.CRM.Customer do
      allow_nil? false
      domain Framework.CRM
    end
  end
