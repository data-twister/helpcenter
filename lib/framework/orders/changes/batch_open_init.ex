defmodule Framework.Orders.Changes.BatchOpenInit do
  @moduledoc false
  ...
  def change(changeset, _opts, _ctx) do
    ...
    item_id = Changeset.get_attribute(changeset, :item_id)
    planned_qty = Changeset.get_attribute(changeset, :planned_qty) || D.new(0)

    item =
      Catalog.get_item_by_id!(item_id, actor: actor, load: [active_bom: [:rollup]])

    {bom_id, bom_version, components_map} =
      case item.active_bom do
        %BOM{} = bom ->
          {
            bom.id,
            Map.get(bom, :version),
    ...
    code = Framework.Production.Batching.generate_batch_code(item.sku, actor)
