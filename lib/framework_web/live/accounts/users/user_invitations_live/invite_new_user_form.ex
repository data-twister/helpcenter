# lib/framework_web/live/accounts/users/user_invitations_live/invite_new_user_form.ex
defmodule FrameworkWeb.Accounts.Users.UserInvitationsLive.InviteNewUserForm do
  @moduledoc """
  LiveComponent that renders a form to invite a new user to the current user's tenant.
  ...
  in the template.
  """
  attr :actor, Framework.Accounts.User, required: true
      ...
      <.modal id="invite-user-modal">
        <div class="flex flex-col space-y-1.5 text-center sm:text-left">
          <h3 class="text-xl font-semibold leading-none tracking-tight">
            {gettext("Invite a new member to your tenant")}
          </h3>
          <p class="text-sm text-gray-500">
            {gettext(
              "Invite a new user to your tenant by entering their email address below. They will receive an invitation email with instructions on how to join your tenant."
            )}
          </p>
        </div>
  ...
  defp assign_access_groups(socket) do
    ...
    groups =
      ...
      |> Ash.read!(tenant: actor.current_tenant, authorize?: false)
