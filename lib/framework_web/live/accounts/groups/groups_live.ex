# lib/framework_web/live/accounts/groups/groups_live.ex
defmodule FrameworkWeb.Accounts.Groups.GroupsLive do
  ...
  def render(assigns) do
    ...
    <div class="flex justify-between">
      ...
      <FrameworkWeb.Accounts.Groups.GroupForm.form actor={@current_user} id={Ash.UUIDv7.generate()} />
    </div>
    <%!-- Table groups --%>
    ...
    <FrameworkWeb.Accounts.Groups.GroupForm.form
      :for={group <- @groups}
      actor={@current_user}
      group_id={group.id}
      show_button={false}
  ...
  defp maybe_subscribe(socket) do
    if connected?(socket), do: FrameworkWeb.Endpoint.subscribe("groups")
  ...
  defp get_groups(actor) do
    Ash.read!(Framework.Accounts.Group, actor: actor)
  end
end
