defmodule Framework.KnowledgeBase.CategoryTest do
  ...
  describe "Knowledge Base Category Tests" do
    test "Can create a category" do
      user = create_user()
      # Create a category and expect to user current_tenant as the
      # tenant databsae for this query
      cat_attrs = %{name: "Billing", description: "testing"}

      category =
        Framework.KnowledgeBase.Category
        |> Ash.Changeset.for_create(:create, cat_attrs, actor: user)
        |> Ash.create!()

      # Confirm that category's tenant is the same as the user current_tenant
      assert user.current_tenant == Ash.Resource.get_metadata(category, :tenant)
    ...
    test "Can filter categories" do
      user = create_user()
      create_categories(user.current_tenant)

      require Ash.Query

      assert Framework.KnowledgeBase.Category
    ...
    test "Can destroy an existing Category" do
      ...
      refute Framework.KnowledgeBase.Category
             |> Ash.Query.filter(name == "Approvals and Workflows")
             |> Ash.exists?(actor: user)
    end
    ...
    test "An article can be added to an existing category" do
      ...
      user = create_user()
      category = get_category(user.current_tenant)
    ...
    test "Category can be retrieved with related articles" do
      ...
      category = get_category(user.current_tenant)
      articles = create_articles(category, user.current_tenant)
    ...
    test "articles_count aggregate can be loaded on the category" do
      ...
      category = get_category(user.current_tenant)
      create_articles(category, user.current_tenant)
    ...
    test "'categories' pubsub event is published on create" do
      ...
      user = create_user()
      FrameworkWeb.Endpoint.subscribe("categories")
    ...
    test "Global preparations works as expected" do
      ...
      assert Framework.KnowledgeBase.Category
             |> Framework.Preparations.LimitTo5.prepare([], [])
             |> Framework.Preparations.MonthToDate.prepare([], [])
             |> Framework.Preparations.OrderByMostRecent.prepare([], [])
    ...
    test "Slug change generates slug successfully" do
      ...
      {:ok, category} =
        Ash.create(Framework.KnowledgeBase.Category, params, actor: user)
