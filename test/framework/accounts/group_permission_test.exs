# test/framework/accounts/group_permission_test.exs
defmodule Framework.Accounts.GroupPermissionTest do
  ...
  describe "Access Group Permission Tests" do
    test "Permission can be added to a group" do
      ...
      group = Ash.create!(Framework.Accounts.Group, group_attrs, actor: user)
      ...
      group_perm =
        Ash.create!(Framework.Accounts.GroupPermission, perm_attr, actor: user, load: [:group])

      # Confirm that the association happened and in the right tenant
      assert user.current_tenant == Ash.Resource.get_metadata(group_perm, :tenant)
      ...
      assert group_perm.resource |> String.to_existing_atom() == Framework.KnowledgeBase.Category
